...
''' this function/script fits and calculates SI related values by each Jackknife.
 Similar to previous interation, but partially works with new keyword paradigm, compatible with old keys
 calculates r_est multiple times for later Wilcoxon test'''
...
def single_oddball_processing(cellid, batch, modelname, force_rerun=False, save_in_DB=False):
    ...
    # defines a mapping from the new keywords into the old keywords to pull the adecuate midway cache

    map_to_old = {'odd.1_fir.2x15-lvl.1_basic-nftrial_si.jk-est.jal-val.jal':
    ...
    if modelname in map_to_old.keys():
        midway_modelname = map_to_old[modelname]
        old_names = True
    else:
    ...
    midway_cache = '/auto/users/mateo/oddball_results/{0}/{1}/{2}/'.format(
        batch, cellid, midway_modelname.rsplit('_', 1)[0])
    if os.path.exists(midway_cache) and force_rerun == False:
        ...
        if old_names == True:
            # changes the meta to be consisten with the new modelname
            for key in meta.keys():
                ctx['modelspecs'][0][0]['meta'][key] = meta[key]
        ...
        # evaluates from loading to fitting evaluates for chaching
